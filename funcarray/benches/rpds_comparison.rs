...
use nickel_lang_funcarray::Vector;
...
pub fn reverse_count(c: &mut Criterion) {
    ...
    group.bench_function("ours 10000, N=8", |b| {
        b.iter(|| black_box(vec8.rev_iter().count()));
    });

    group.bench_function("ours 10000, N=32", |b| {
        b.iter(|| black_box(vec32.rev_iter().count()));
    });

    group.bench_function("ours 10000, N=64", |b| {
        b.iter(|| black_box(vec64.rev_iter().count()));
    });

    group.bench_function("rpds 10000", |b| {
        b.iter(|| black_box(rpds.iter().rev().count()));
    });
}
