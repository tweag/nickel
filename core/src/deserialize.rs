...
use crate::{
    bytecode::value::{lens::ValueLens, Array, InlineValue, NickelValue, ValueContent, ValueContentRef, StringBody},
    error::{self, NullReporter},
    eval::cache::CacheImpl,
    identifier::LocIdent,
    program::{Input, Program},
    term::{record::Field, IndexMap, Term},
};
...
impl<'de> serde::Deserializer<'de> for NickelValue {
        ...
        V: Visitor<'de>,
    {
        match self.content() {
            ValueContent::Inline(lens) => match lens.take() {
                ...
                InlineValue::EmptyArray => visitor.visit_seq(ArrayDeserializer::new(Array::default())),
            ...
            other => Err(RustDeserializationError::UnimplementedType {
                occurred: other.type_of().unwrap_or("Other").to_owned(),
            }),
        }
    }
    ...
    /// deserialize `NickelValue::Enum` tags or `NickelValue::Record`s with a single item.
    ...
    /// Deserialize `NickelValue::String` as char
    ...
    /// Deserialize `NickelValue::String` as str
    ...
    /// Deserialize `NickelValue::String` as String
    fn deserialize_string<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>,
    {
        match self.as_string() {
            Some(StringBody(s)) => visitor.visit_string(s.into_inner()),
            None => Err(RustDeserializationError::InvalidType {
    ...
    /// Deserialize `NickelValue::String` as String or `NickelValue::Array` as array,
    ...
    /// Deserialize `NickelValue::Str` as String or `NickelValue::Array` as array,
    ...
    /// Deserialize `NickelValue::Null` as `()`.
    ...
    /// Deserialize `NickelValue::Array` as `Vec<T>`.
    fn deserialize_seq<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>,
    {
        match self.as_array() {
            Some(body) => visit_array(body.array, visitor),
            None => Err(RustDeserializationError::InvalidType {
    ...
    /// Deserialize `NickelValue::Record` as `HashMap<K, V>`.
    fn deserialize_map<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>,
    {
        match self.as_record() {
            Some(record) => visit_record(record.0.fields, visitor),
            None => Err(RustDeserializationError::InvalidType {
    ...
    /// Deserialize `NickelValue::Record` as `struct`.
...
mod tests {
    ...
    use nickel_lang_utils::{
        nickel_lang_core::{
            deserialize::RustDeserializationError, error::NullReporter, term::NickelValue,
        },
