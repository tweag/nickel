...
pub fn get_uop_type(
    ...
    Ok(match op {
        ...
        UnaryOp::IfThenElse => {
            ...
            (
                mk_uniftype::bool(),
                mk_buty_arrow!(branches.clone(), branches.clone(), branches),
            )
        }
        ...
        UnaryOp::RecordAccess(id) => {
            ...
            (mk_buty_record!((*id, res.clone()); rows), res)
        }
        ...
        UnaryOp::Seq | UnaryOp::DeepSeq => {
            ...
            (fst, mk_buty_arrow!(snd.clone(), snd))
        }
        ...
        UnaryOp::Trace => {
            let ty = state.table.fresh_type_uvar(var_level);
            (mk_uniftype::str(), mk_buty_arrow!(ty.clone(), ty))
        }
        // Morally: Lbl -> Lbl
        // Actual: Dyn -> Dyn
...
pub fn get_bop_type(
    ...
    Ok(match op {
        ...
        BinaryOp::ContractCheck => (
            mk_uniftype::dynamic(),
            mk_uniftype::dynamic(),
            mk_buty_arrow!(mk_uniftype::dynamic(), custom_contract_ret_type()),
        ),
...
pub fn custom_contract_ret_type() -> UnifType {
    mk_buty_enum!(("Ok", mk_uniftype::dynamic()), ("Error", error_data_type()))
}
...
fn error_data_type() -> UnifType {
    ...
    UnifType::concrete(TypeF::Contract((error_data.build(), TermEnv::new())))
}
