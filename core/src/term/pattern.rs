...
pub mod compile {
    ...
    /// The special field used to capture the rest of a record pattern. Although it is stored in
    /// the bindings, it's not an actual bindings and is filtered out at the end of the
    /// corresponding record pattern.
    const REST_FIELD: &str = "$rest";
    ...
    /// Generate a Nickel expression which update the `REST_FIELD` field of the working dictionary,
    /// putting `value_id` minus the current field.
    ///
    /// ```nickel
    /// %record_insert% "<REST_FIELD>"
    ///   (%record_remove% "<REST_FIELD>" bindings_id)
    ///   (%record_remove% "<field>" value_id)
    /// ```
    fn update_rest(field: LocIdent, value_id: LocIdent, bindings_id: LocIdent) -> RichTerm {
        let value_shrinked = make::op2(
    ...
    impl CompilePart for RecordPattern {
        ...
        // the bindings in a special field that can't conflict with a valid identifier (at least as
        // long as we don't allow raw identifiers in patterns): `$rest`. Note that this is an
        ...
        //      let prev_bindings_id = cont in
        //      let value_id = %static_access(field)% (%static_access(REST_FIELD)% prev_bindings_id)
        //      let new_bindings_id = <update_rest(field, value_id, new_bindings_id)> in
        //
        ...
        fn compile_part(&self, value_id: LocIdent, bindings_id: LocIdent) -> RichTerm {
            ...
            // <fold (field, value) in fields / cont is the accumulator>
            // if %field_is_defined% field value_id then
            //   let bindings_id = cont in
            //   let value_id = %static_access(field)% (%static_access(REST_FIELD)% bindings_id)
            //   let bindings_id = <update_rest(field, value_id, new_bindings_id)> in
            //
            //   if new_bindings_id == null then
            //     null
            //   else
            ...
            let fold_block: RichTerm =
                self.patterns
                    ...
                    .fold(Term::Var(bindings_id).into(), |cont, field_pat| {
                        ...
                        //   null
                        // else
                        //   <field.compile_part(value_id, bindings_id)>
                        let inner_if = make::if_then_else(
                            make::op2(BinaryOp::Eq(), Term::Var(bindings_id), Term::Null),
                            Term::Null,
                            field_pat.pattern.compile_part(value_id, bindings_id),
                        );
                        ...
                        let updated_bindings_let = make::let_in(
                            bindings_id,
                            update_rest(field, value_id, bindings_id),
                            inner_if,
                        );

                        // let value_id = %static_access(field)% (%static_access(REST_FIELD)% bindings_id)
                        let value_let = make::let_in(
                            value_id,
                            make::op1(
                        ...
                        // let bindings_id = cont in <value_let>
                        let outer_bindings_let = make::let_in(bindings_id, cont, value_let);

                        // if <has_field> then <outer_binding_let> else null
                        make::if_then_else(has_field, outer_bindings_let, Term::Null)
                    });
