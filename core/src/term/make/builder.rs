...
fn elaborate_field_path<I>(path: I, content: record::Field) -> (Ident, record::Field)
where
...
where
    I: IntoIterator<Item = (Ident, record::Field)>,
{
...
mod tests {
    ...
    fn trivial() {
        ...
        assert_eq!(
            t,
            build_record(
                vec![(Ident::new("foo"), term(Term::Str("bar".to_owned().into())))],
                Default::default()
            )
            .into()
        );
    ...
    fn from_iter() {
        ...
        assert_eq!(
            ...
            build_record(
                vec![
                    (Ident::new("foo"), term(Term::Null)),
                    (Ident::new("bar"), term(Term::Null)),
                ],
                Default::default()
            )
            .into()
    ...
    fn fields() {
        ...
        assert_eq!(
            ...
            build_record(
                vec![
                    (Ident::new("foo"), term(Term::Str("foo".into()))),
                    (Ident::new("bar"), term(Term::Str("bar".into()))),
                ],
                Default::default()
            )
            .into()
