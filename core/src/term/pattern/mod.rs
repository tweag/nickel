...
impl From<ConstantPatternData> for Term {
    fn from(data: ConstantPatternData) -> Self {
        match data {
            ConstantPatternData::Bool(b) => Term::Bool(b),
            ConstantPatternData::Number(n) => Term::Num(n),
            ConstantPatternData::String(s) => Term::Str(s),
            ConstantPatternData::Null => Term::Null,
        }
    }
}
...
impl ElaborateContract for PatternData {
    fn elaborate_contract(&self) -> Option<LabeledType> {
        match self {
            ...
            PatternData::Array(pat) => pat.elaborate_contract(),
            PatternData::Constant(pat) => pat.elaborate_contract(),
            PatternData::Or(_) => panic!("can't elaborate a contract for or-patterns"),
            PatternData::Wildcard => None,
        }
    }
}
...
impl ElaborateContract for ArrayPattern {
    fn elaborate_contract(&self) -> Option<LabeledType> {
        let typ = Type {
            // We don't have an easy way to specify an heterogenuous array contract, and it would
            // be of dubious utility. So we just use the `Dyn` contract for now.
            typ: TypeF::Array(Box::new(TypeF::Dyn.into())),
            pos: self.pos,
        };
...
impl ElaborateContract for ConstantPattern {
    fn elaborate_contract(&self) -> Option<LabeledType> {
        let typ = Type {
            typ: TypeF::Flat(mk_app!(
                internals::stdlib_contract_equal(),
                RichTerm::new(self.data.clone().into(), self.pos)
            )),
            pos: self.pos,
        };
