...
pub trait IntoDiagnostics {
    ...
    /// # Arguments
    ///
    /// - `cache`: to know why it needs a mutable reference to the [SourceCache], see
    ///   `label_alt`.
...
/// ```
///
/// The reason for the mutable reference to the source cache is that codespan does not let you
/// annotate something that is not in its collection of files, which is managed by the source
/// cache: you can't provide a raw snippet, you need to provide a `FileId` referring to a file.
    ...
    fn into_diagnostics(
        ...
        match self {
                ...
                let labels = match (pos_orig.into_opt(), t.pos.into_opt()) {
                    ...
                    (Some(span_orig), Some(_)) => {
                        vec![
                            ...
                            secondary_term(&t, cache).with_message("evaluated to this"),
                        ]
                    }
                    (Some(span), None) => {
                    ...
                    (None, None) => {
                        vec![primary_term(&t, cache).with_message(label)]
                    }
                };
            ...
            EvalError::NotAFunc(t, arg, pos_opt) => vec![Diagnostic::error()
                ...
                .with_labels(vec![
                    primary_term(&t, cache)
                        .with_message("this term is applied, but it is not a function"),
                    secondary_alt(pos_opt, format!("({}) ({})", t, arg), cache)
            ...
            EvalError::MergeIncompatibleArgs {
                ...
                let mut labels = vec![
                    primary_term(&left_arg, cache).with_message("cannot merge this expression"),
                    primary_term(&right_arg, cache).with_message("with this expression"),
                ];
                ...
                let labels = match eq_pos {
                    TermPos::Original(pos) | TermPos::Inherited(pos) if eq_pos != t.pos => {
                        vec![
                            ...
                            secondary_term(&t, cache)
                                .with_message("problematic argument evaluated to this"),
                        ]
                    }
                    _ => vec![primary_term(&t, cache).with_message(label)],
                };
            ...
            EvalError::QueryNonRecord { pos, id, value } => {
                ...
                let label = if let Some(span) = pos.into_opt() {
                    ...
                    primary_term(&value, cache).with_message(label)
                };
...
mod blame_error {
    ...
    pub fn path_span(cache: &mut SourceCache, path: &[ty_path::Elem], ty: &Type) -> PathSpan {
        ...
        ty_path::span(path.iter().peekable(), ty)
            .or_else(|| {
                ...
                let file_key = cache.insert_generated(Source::Memory {
                    source: type_pprinted,
                });
    ...
    fn into_diagnostics(
        ...
        match self {
            ...
            TypecheckError::ArrowTypeMismatch {
                ...
                let PathSpan {
                    span: expd_span, ..
                } = blame_error::path_span(cache, &type_path, &expected);
                let PathSpan {
                    span: actual_span, ..
                } = blame_error::path_span(cache, &type_path, &inferred);
    ...
    fn into_diagnostics(
        ...
        match self {
            ExportError::NotAString(rt) => vec![Diagnostic::error()
                ...
                .with_labels(vec![primary_term(&rt, cache)])],
            ...
            ExportError::NumberOutOfRange { term, value } => vec![Diagnostic::error()
                ...
                .with_labels(vec![primary_term(&term, cache)])
                .with_notes(vec![format!(
