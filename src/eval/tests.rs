...
fn imports() {
    ...
    vm.reset();
    assert_eq!(
        vm.eval(mk_import_two, &Environment::<EC>::new(),)
            .map(Term::from)
            .unwrap(),
        Term::Num(2.0)
    );
    ...
    vm.reset();
    assert_eq!(
        vm.eval(mk_import_lib.unwrap(), &Environment::<EC>::new(),)
            .map(Term::from)
            .unwrap(),
        Term::Bool(true)
    );
...
#[test]
fn initial_env() {
    let mut initial_env = Environment::<EC>::new();
...
fn mk_env(bindings: Vec<(&str, RichTerm)>, eval_cache: &mut EC) -> Environment<EC> {
    bindings
        .into_iter()
        .map(|(id, t)| {
            (
