...
fn fold_normal(c: &mut Criterion) {
    bench("fold normal 100", env!("CARGO_MANIFEST_DIR"), "lists/fold", Some("right.strings"), 100, EvalMode::Normal, c);
}

fn fold_deepseq(c: &mut Criterion) {
    bench("fold deepseq 100", env!("CARGO_MANIFEST_DIR"), "lists/fold", Some("right.strings"), 100, EvalMode::DeepSeq, c);
}
